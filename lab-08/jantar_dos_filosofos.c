...
*   descri??o.: Um clássico da programação paralela, Dijkstra (1965)
*
*
...
void pegar_hashi ( int id_filosofo )
{
	pthread_mutex_lock( &(mutex) );					//Entra na regi?o crítica
...
void devolver_hashi ( int id_filosofo )
{
	...
	pthread_mutex_unlock ( &(mutex) );	//Sai da regi?o crítica
}
//-------------------------------------------------------------------------------
/*!
...
void intencao ( int id_filosofo )
{
	...
	if( (estado[id_filosofo] == FAMINTO) &&	//Se o filósofo está come fome
		...
		(estado[DIREITA] != COMENDO ) )		//Se o vizinho da direita n?o está comendo
	{
